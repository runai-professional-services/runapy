...
class ResourcesMemory(BaseModel):
    """
    Pydantic class model representing ResourcesMemory.
    ...
    Parameters:
        ...
        units: See model str for more information.
    Example:
        ```python
        ResourcesMemory(
            deserved=1000,
    ...
    def units_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["Mib", "MB", "GB"]):
            raise ValueError("must be one of enum values ('Mib', 'MB', 'GB')")
        return value
