...
class NimServiceMultiNode(BaseModel):
    """
    Pydantic class model representing Defines whether the NIM service runs as a multi-node deployment. If workers is set to 1 or more, the service runs in multi-node..

    Parameters:
        ```python
        workers: Optional[int]
        ```
        workers: Specifies the number of worker nodes to use when running the NIM service in multi-node.
    Example:
        ```python
        NimServiceMultiNode(
            workers=3
    ...
    workers: Optional[Annotated[int, Field(le=1000, strict=True, ge=1)]] = Field(
        default=None,
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of NimServiceMultiNode from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of NimServiceMultiNode from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({"workers": obj.get("workers")})
        return _obj
