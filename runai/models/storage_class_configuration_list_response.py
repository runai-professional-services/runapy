...
class StorageClassConfigurationListResponse(BaseModel):
    """
    Pydantic class model representing StorageClassConfigurationListResponse.

    Parameters:
        ```python
        storage_class_configurations: List[StorageClassConfiguration]
        next: int
        ```
        storage_class_configurations: A list of storage class configuration objects. Each object defines the configuration details for a specific storage class.
        next: See model int for more information.
    Example:
        ```python
        StorageClassConfigurationListResponse(
            storage_class_configurations=[
                    runai.models.storage_class_configuration.StorageClassConfiguration(
                        id = '',
                        tenant_id = 1001,
                        scope_type = 'cluster',
                        scope_id = '',
    ...
    storage_class_configurations: List[StorageClassConfiguration] = Field(
        ...
        alias="storageClassConfigurations",
    )
    next: Optional[StrictInt] = None
    __properties: ClassVar[List[str]] = ["storageClassConfigurations", "next"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of StorageClassConfigurationListResponse from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in storage_class_configurations (list)
        _items = []
        if self.storage_class_configurations:
            for _item_storage_class_configurations in self.storage_class_configurations:
                if _item_storage_class_configurations:
                    _items.append(_item_storage_class_configurations.to_dict())
            _dict["storageClassConfigurations"] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of StorageClassConfigurationListResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "storageClassConfigurations": (
                    [
                        StorageClassConfiguration.from_dict(_item)
                        for _item in obj["storageClassConfigurations"]
                    ]
                    if obj.get("storageClassConfigurations") is not None
                    else None
                ),
                "next": obj.get("next"),
            }
