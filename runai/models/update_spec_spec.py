...
class UpdateSpecSpec(BaseModel):
    """
    Pydantic class model representing UpdateSpecSpec.

    Parameters:
        ```python
        replicas: Optional[int]
        ```
        replicas: Specifies the number of leader-worker sets to deploy. Each replica represents a group consisting of one leader pod and multiple worker pods. For example, setting replicas to 3 will create 3 independent groups, each with its own leader and corresponding set of workers.
    Example:
        ```python
        UpdateSpecSpec(
            replicas=2
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of UpdateSpecSpec from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of UpdateSpecSpec from a dict"""
        if obj is None:
            return None
