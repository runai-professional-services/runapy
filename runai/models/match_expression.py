...
class MatchExpression(BaseModel):
    """
    Pydantic class model representing A selector that contains values, a key, and an operator that relates the key and values..
    ...
    Parameters:
        ...
        values: List[str]
        ```
        key: The label key that the selector applies to (mandatory).
        operator: See model MatchExpressionOperator for more information.
        values: An array of string values. If the operator is In or NotIn, the values array must be non-empty. If the operator is Exists or DoesNotExist, the values array must be empty. If the operator is Gt or Lt, the values array must have a single element, which will be interpreted as an integer.
    Example:
        ```python
        MatchExpression(
