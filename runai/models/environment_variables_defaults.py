...
class EnvironmentVariablesDefaults(BaseModel):
    ...
    Parameters:
        ```python
        instances: Optional[List[EnvironmentVariable1]]
        ```
        instances: Set of environment variables to populate into the container running the workspace.
    Example:
        ```python
    ...
    instances: Optional[List[Optional[EnvironmentVariable1]]] = Field(
        default=None,
        ...
        _obj = cls.model_validate(
            {
                "instances": (
                    [
                        EnvironmentVariable1.from_dict(_item)
