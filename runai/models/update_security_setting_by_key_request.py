...
class UpdateSecuritySettingByKeyRequest(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = UpdateSecuritySettingByKeyRequest.model_construct()
        error_messages = []
        ...
        if not isinstance(v, BrowserSessionTimeoutSetting):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `BrowserSessionTimeoutSetting`"
            )
        else:
            return v

        # validate data type: LogoutRedirectUriSetting
        if not isinstance(v, LogoutRedirectUriSetting):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `LogoutRedirectUriSetting`"
            )
        else:
            return v
