...
class NgcApiKeyCreationSpec(BaseModel):
    """
    Pydantic class model representing NgcApiKeyCreationSpec.

    Parameters:
        ```python
        ngc_api_key: str
        ```
        ngc_api_key: The NGC API key value.
    Example:
        ```python
    ...
    ngc_api_key: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(
        default=None, description="The NGC API key value."
    )
    __properties: ClassVar[List[str]] = ["ngc_api_key"]

    @field_validator("ngc_api_key")
    def ngc_api_key_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^[-._A-Za-z0-9]+$", value):
            raise ValueError(
                r"must validate the regular expression /^[-._A-Za-z0-9]+$/"
            )
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of NgcApiKeyCreationSpec from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of NgcApiKeyCreationSpec from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({"ngc_api_key": obj.get("ngc_api_key")})
        return _obj
